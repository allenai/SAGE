...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if IS_MOLMO2:
            source_key = "raw_prompt"
            dest_key = "prompt"
        else:
            source_key = "raw_prompt_ids"
            dest_key = "prompt_token_ids"
        
        if "multi_modal_data" in non_tensor_batch:
            vllm_inputs = []
            for raw_prompt_ids, multi_modal_data in zip(
                non_tensor_batch.pop(source_key), non_tensor_batch.pop("multi_modal_data"), strict=True
            ):
                vllm_inputs.append({dest_key: raw_prompt_ids, "multi_modal_data": multi_modal_data})
        else:
            vllm_inputs = [
                {dest_key: raw_prompt_ids} for raw_prompt_ids in non_tensor_batch.pop(source_key)
            ]
