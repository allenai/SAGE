...
from sage.src.models.qwen_vl.vision_process import fetch_image
from sage.src.models.qwen_vl.vision_process import fetch_video, _load_video_inputs, _save_video_inputs
...
def process_video(
    ...
    if use_cache:
        cached_data = _load_video_inputs(video["video"])
    else:
        cached_data = None
    if cached_data is not None:
        # Use cached data
        video_input = cached_data["video_inputs"][0]
    else:
        # Process video normally and cache the result
        video_input, video_sample_fps = fetch_video(video, return_video_sample_fps=True)
        # Cache the processed video inputs
        video_kwargs = {"fps": [video_sample_fps], "video_paths": [video["video"]]}
        _save_video_inputs([video_input], video_kwargs, video["video"])
