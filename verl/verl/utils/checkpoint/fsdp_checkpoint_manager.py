            ...
            if self.rank == 0:
                ...
                with init_empty_weights():
                    try:
                        ...
                        from sage.src.models.molmo2 import Molmo2ForConditionalGeneration, Molmo2Config
                        # Convert dict config to Molmo2Config object
                        if isinstance(model_config, dict):
                            config = Molmo2Config.from_dict(model_config)
                        else:
                            config = model_config
                        save_model = Molmo2ForConditionalGeneration(config)
                        # Set the dtype to match the original from_config call
                        save_model = save_model.to(torch.bfloat16)
