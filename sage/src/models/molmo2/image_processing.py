...
class Molmo2ImageProcessor(BaseImageProcessor):
    ...
    def pad_arrays(self, arrays: List[np.ndarray], pad_value: float = -1) -> np.ndarray:
        row_shape = np.stack([x.shape for x in arrays], 0).max(0)
        padded_arr = np.full(
        ...
        for ix, arr in enumerate(arrays):
            a = arr[:row_shape[0]]
            slices = tuple(slice(None, dim) for dim in a.shape)
            padded_arr[(ix,) + slices] = a
        return padded_arr
    ...
    def pad_for_batching(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        pixel_values = self.pad_arrays(data["pixel_values"])
        image_token_pooling = self.pad_arrays(data["image_token_pooling"])
